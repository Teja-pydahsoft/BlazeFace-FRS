"""
High-performance multi-threaded face detection system
...
class ProcessingMode(Enum):
    ...
    HIGH_FPS = "high_fps"  # Fast processing, lower accuracy
    BALANCED = "balanced"  # Balanced speed and accuracy
    HIGH_ACCURACY = "high_accuracy"  # Slower processing, higher accuracy

class HighPerformanceDetector:
    """High-performance multi-threaded face detector"""
    ...
    def __init__(self, 
        ...
        # Initialize detection zones based on common NVR camera layouts
        self._initialize_default_zones()
    
    def _initialize_default_zones(self):
        """Initialize default detection zones for NVR cameras"""
    ...
    def _worker_loop(self):
        ...
        while self.running:
            try:
                ...
                if self.processing_mode == ProcessingMode.HIGH_FPS:
                    faces = self._process_high_fps(frame, detector)
                elif self.processing_mode == ProcessingMode.HIGH_ACCURACY:
                    faces = self._process_high_accuracy(frame, detector)
                else:  # BALANCED
                    faces = self._process_balanced(frame, detector)
    ...
    def _process_high_fps(self, frame: np.ndarray, detector) -> List[Tuple[int, int, int, int, float]]:
        """High FPS processing - fast but less accurate"""
    ...
    def _process_balanced(self, frame: np.ndarray, detector) -> List[Tuple[int, int, int, int, float]]:
        """Balanced processing - good speed and accuracy"""
        return detector.detect_faces(frame)
    
    def _process_high_accuracy(self, frame: np.ndarray, detector) -> List[Tuple[int, int, int, int, float]]:
        """High accuracy processing - slower but more accurate"""
        # Process at full resolution
        faces = detector.detect_faces(frame)
        
        # Additional validation for high accuracy
        validated_faces = []
        for face in faces:
            x, y, w, h, conf = face
            
            # Additional checks for high accuracy
            if w > 20 and h > 20 and conf > 0.5:
                # Check if face is in reasonable position
                center_y = y + h // 2
                if center_y < frame.shape[0] * 0.8:  # Not too low in frame
    ...
    def draw_detection_zones(self, frame: np.ndarray) -> np.ndarray:
        ...
        for zone in self.detection_zones:
            ...
            # Draw zone criteria
            criteria_text = f"Size: {zone.min_face_size}-{zone.max_face_size}, Conf: {zone.confidence_threshold:.1f}"
            cv2.putText(frame_with_zones, criteria_text,
                       (zone.x, zone.y + zone.height + 20),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, zone.color, 1)

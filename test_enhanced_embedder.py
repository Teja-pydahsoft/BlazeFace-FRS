"""
Test enhanced facial feature recognition
"""
...
def test_enhanced_embedder():
    """Test enhanced facial feature recognition"""
    print("Testing Enhanced Facial Feature Recognition...")
    
    try:
        embedder = EnhancedFaceEmbedder()
    except Exception as e:
        print(f"❌ Failed to initialize EnhancedFaceEmbedder: {e}")
        return
    ...
    cv2.imwrite("enhanced_face1.jpg", face1)
    cv2.imwrite("enhanced_face2.jpg", face2)
    cv2.imwrite("enhanced_face3.jpg", face3)
    print("Saved enhanced faces as enhanced_face1.jpg, enhanced_face2.jpg, enhanced_face3.jpg")
    ...
    if emb1 is None or emb2 is None or emb3 is None:
        print("❌ Failed to generate enhanced embeddings")
        return
    
    print(f"Enhanced embedding 1 shape: {emb1.shape}, norm: {np.linalg.norm(emb1):.4f}")
    print(f"Enhanced embedding 2 shape: {emb2.shape}, norm: {np.linalg.norm(emb2):.4f}")
    print(f"Enhanced embedding 3 shape: {emb3.shape}, norm: {np.linalg.norm(emb3):.4f}")
    ...
    print("- Enhanced eye characteristics (shape, gradient, edges)")
    print("- Enhanced nose characteristics (shape, gradient, edges)")
    print("- Enhanced mouth characteristics (shape, gradient, edges)")
    print("- Enhanced jawline characteristics (contour, shape)")
    print("- Enhanced forehead characteristics (texture, shape)")
    print("- Enhanced cheek characteristics (texture, shape)")
    print("- Enhanced symmetry analysis (left-right comparison)")
    print("- Enhanced texture features (LBP, Gabor filters)")
    print("- Enhanced edge features (Canny, Laplacian)")

if __name__ == "__main__":
    test_enhanced_embedder()
